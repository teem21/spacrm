...
const { Gem, Settings, Calendar, BookOpen, LayoutDashboard, Loader2, Plus, Trash2, ChevronLeft, ChevronRight, ChevronUp, ChevronDown, Check, X, Moon, Phone, Search } = lucide;
 ...
 * @property {"booked"|"completed"|"cancelled"|"no-show"} status
...
function ScheduleFields({ config, onChange }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      <NumberInput
        label="Количество мастеров" value={config.therapistCount}
...
function SettingsScreen({ salons, onSalonsChange, onShowToast, onReset, onImportComplete }) {
...
const CATEGORY_LABEL  = { massage: "Массаж", sauna: "Сауна", peeling: "Пиллинг" };
const CATEGORY_OPTIONS = [
  { value: "massage", label: "💆 Массаж"  },
  { value: "sauna",   label: "🧖 Сауна"   },
  { value: "peeling", label: "✨ Пиллинг" },
];
  ...
  if (therapistSegs.length > 0) {
    ...
    for (let m = minM; m < maxM; m += 15) {
      ...
      if (existingLoad + newLoad > salon.therapistCount) {
        ...
        errors.push({ field: "therapists", message: `Недостаточно мастеров (нужно ${newLoad}, свободно ${free} в ${slotS})` });
        break;
      }
    }
  }

  // 7. Beds check — pair in single-bed room
  if (booking.clientCount === 2) {
    for (const seg of booking.segments) {
      if (seg.resourceType === "room" && seg.roomId) {
        const room = salon.rooms.find(r => r.id === seg.roomId);
        if (room && room.beds < 2) {
          errors.push({ field: "room", message: `${room.name} имеет ${room.beds} кровать — нужно 2 для пары` });
          break;
        }
      }
    }
...
function BookingModal({ salon, procedures, combos, initialDate, initialTime, initialRoomId, onSave, onClose }) {
  ...
  const [procedureId, setProcedureId] = useState(activeProcedures[0]?.id || "");
  ...
  const selectedProc = activeProcedures.find(p => p.id === procedureId) || activeProcedures[0];
  ...
  const maxClients = hasSaunaInSelection ? (salon.saunaCapacity || 4) : 4;
  ...
  const therapistCount = (() => {
    if (bookingType === "single") return clientCount * (selectedProc?.therapistsRequired || 0);
    if (!selectedCombo) return 0;
    let total = 0;
  ...
  const totalPrice = bookingType === "single"
    ? (selectedProc?.price || 0) * clientCount
...
function ScheduleScreen({ activeSalonId, salons, procedures, combos, onShowToast }) {
  const salon = salons.find(s => s.id === activeSalonId);
  const [selectedDate, setSelectedDate] = useState(new Date());
  const [monthBookings, setMonthBookings] = useState([]);
  const [bookingModal, setBookingModal] = useState(null); // null | { initialTime, initialRoomId }
  ...
  const handleBookingCreated = (_booking, bookingYm, updatedBookings) => {
    if (bookingYm === toYM(selectedDate)) setMonthBookings(updatedBookings);
    setBookingModal(null);
    if (onShowToast) onShowToast("Запись создана");
  };

  const handleStatusChange = async (bookingId, newStatus) => {
    const curYm = toYM(selectedDate);
    const all = await Storage.get(KEYS.bookings(activeSalonId, curYm)) || [];
    const updated = all.map(b => b.id === bookingId ? { ...b, status: newStatus } : b);
    await Storage.set(KEYS.bookings(activeSalonId, curYm), updated);
  ...
  const isDayOff    = JS_DAY_KEY[selectedDate.getDay()] === salon.dayOff;
  ...
  const nowLeft = (isToday && nowM >= wStartM && nowM <= wEndM) ? (nowM - wStartM) / 30 * CELL_W : null;

  // Therapist usage per slot
  ...
  const dayLabel = () => {
    ...
    return `${RU_WEEKDAY[d.getDay()]}, ${d.getDate()} ${RU_MONTH[d.getMonth()]} ${d.getFullYear()}`;
  };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
        {/* Day nav */}
        <button onClick={() => setSelectedDate(d => addDays(d, -1))} style={btnStyle()}>
          <ChevronLeft size={15} />
        </button>
        <span style={{ color: C.textMain, fontSize: 14, fontWeight: 500, minWidth: 260, textAlign: "center" }}>
          {dayLabel()}
        </span>
        <button onClick={() => setSelectedDate(d => addDays(d, 1))} style={btnStyle()}>
        ...
        <input type="date" value={dateStr}
          onChange={e => { if (e.target.value) setSelectedDate(parseLocal(e.target.value)); }}
          style={{ ...inputStyle(), width: 160, height: 34, fontSize: 12, colorScheme: "dark", cursor: "pointer" }}
        />

        {/* Today shortcut */}
        {!isToday && (
          <button onClick={() => setSelectedDate(new Date())} style={btnStyle({ fontSize: 12 })}>
            Сегодня
          </button>
        )}
      ...
      {isDayOff ? (
        <div style={{
          display: "flex", flexDirection: "column", alignItems: "center",
          justifyContent: "center", height: "50vh", gap: 12, color: C.textSub,
        }}>
          <Moon size={48} color={C.textSub} />
          <p style={{ margin: 0, fontSize: 16, color: C.textMain }}>Выходной день</p>
          <p style={{ margin: 0, fontSize: 13 }}>{RU_WEEKDAY[selectedDate.getDay()]} — нерабочий день</p>
        </div>
      ) : (
                    ...
                    <div key={i} style={{
                      ...
                      <div style={{ display: "flex", gap: 1, flexWrap: "wrap", justifyContent: "center", maxWidth: CELL_W - 8 }}>
                        {Array.from({ length: Math.min(u.total, 12) }).map((_, j) => (
                          <div key={j} style={{
                            width: sqSize, height: sqSize, borderRadius: 1,
...
function DashboardScreen({ salons }) {
  ...
  const dateRange = (() => {
    ...
    if (period === "week") return { from: toDateStr(addDays(new Date(), -6)), to: today };
    // month
    const d = new Date();
  ...
  const computeKpi = (bookings, salonFilter) => {
    ...
    const totalClients = bks.reduce((a, b) => a + (b.clientCount || 1), 0);
    const revenue = completed.reduce((a, b) => a + (b.totalPrice || 0), 0);
    ...
    // Days in period
    const daySet = new Set(bks.map(b => b.date));
    ...
    for (const sal of targetSalons) {
      ...
      therTotalMins += (sal.therapistCount || 1) * wMins * daysCount;
    }
    ...
    return { totalBookings, totalClients, revenue, avgCheck, roomPct, therPct };
  };
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
        ...
        <div style={cardStyle}>
          ...
          <div style={{ fontSize: 11, color: C.textSub }}>Выручка (завершённые)</div>
        </div>
        <div style={cardStyle}>
...
function App() {
  ...
  const [procedures, setProcedures] = useState([]);   // for activeSalonId
  const [combos, setCombos] = useState([]);            // for activeSalonId
  const [bookings, setBookings] = useState([]);        // current month, activeSalonId
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!savedSalons || savedSalons.length === 0) {
        // No data — show onboarding wizard (wizard will save everything)
  ...
  // Called by OnboardingWizard after it has saved salons + procedures to storage
  ...
  // Reload salon data when active salon changes (after initial load)
  ...
  if (needsOnboarding) return <OnboardingWizard onComplete={handleOnboardingComplete} />;
  ...
  return (
    <div style={{
      ...
      {/* Content area */}
      <main style={{
        marginTop: 100, // header (56) + tabbar (44)
        padding: 24,
        maxWidth: 1400,
        margin: "100px auto 0",
        overflowY: "auto",
        ...
        {activeTab !== "settings" && activeTab !== "services" && activeTab !== "schedule" && activeTab !== "journal" && activeTab !== "dashboard" && (
          <ContentPlaceholder tab={activeTab} />
        )}
      </main>
